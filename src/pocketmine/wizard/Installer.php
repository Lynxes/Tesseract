...
LICENSE;
	...
	private function generateBaseConfig(){
		...
		echo "[*] " . $this->lang->get("online_mode_info") . "\n";
		echo "[?] " . $this->lang->get("online_mode") . " (y/N): ";
		$config->set("online-mode", strtolower($this->getInput("y")) == "y");
		
		echo "[?] " . $this->lang->get("level_name") . " (" . self::DEFAULT_LEVEL_NAME . "): ";
		$config->set("level-name", $this->getInput(self::DEFAULT_LEVEL_NAME));
		
		do{
			echo "[?] " . $this->lang->get("level_type") . " (" . self::DEFAULT_LEVEL_TYPE . "): ";
			$type = strtoupper((string) $this->getInput(self::DEFAULT_LEVEL_TYPE));
			if(!in_array($type, self::LEVEL_TYPES)){
				echo "[!] " . $this->lang->get("invalid_level_type") . "\n";
			}
		...
		echo "[?] " . $this->lang->get("announce_player_achievements") . " (y/N): ";
		if(strtolower($this->getInput("n")) === "y"){
